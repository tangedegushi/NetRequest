...
public class LoadingView extends View {
    ...
    private boolean start;//是否一开始就开始动画
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (wMode != MeasureSpec.EXACTLY) {
            wSize = (int) (gap + 2 * (radius1 + radius2) * maxScale + dp2px(1));
        }
        if (hMode != MeasureSpec.EXACTLY) {
            hSize = (int) (2 * Math.max(radius1, radius2) * maxScale + dp2px(1));
        }
        Logger.zzqLog().e("width = " + wSize + "   height = " + hSize + "   mode = " + (wMode >> 30) + "  " + (hMode >> 30));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float ltrX = (getMeasuredWidth() - distance) / 2.0f;
        ltrX = ltrX + (distance * fraction);//当前从左往右的球的X坐标

        float rtlX = (getMeasuredWidth() + distance) / 2.0f;
        ...
        if (fraction <= scaleStartFraction) { //动画进度[0,scaleStartFraction]时，球大小由1倍逐渐缩放至ltrScale/rtlScale倍
            ...
            float scaleFraction = (1 - fraction) / (1 - scaleEndFraction); //百分比转换，[scaleEndFraction,1] -> [1,0]
            ltrBallRadius = ltrInitRadius * (1 + (ltrScale - 1) * scaleFraction);
            rtlBallRadius = rtlInitRadius * (1 + (rtlScale - 1) * scaleFraction);
